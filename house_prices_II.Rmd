...
description: "This is the second .Rmd document from a set of three .Rdm. Code provided is complemetary to the main document ***** that can be found in this repository. This markdown contains the code needed for conducting the analysis of point 3. DATA EXTRACTION AND PREPROCESSING."
---
...
```{r}
class(parks)
class(educational_centers)
class(health_centers)
```

```{r}
map <- leaflet() %>%
  setView(lng = -3.759284, lat = 40.39557, zoom = 10)

map <- map %>%
  addPolygons(data = geodata_districts,
              fillColor = "red",   # Change color as desired
              fillOpacity = 1,    # Adjust opacity as desired
...
# Load the district data and add it to the map
district_layer <- addPolygons(map, data = geodata_districts, 
                                       fillColor = "lightblue",
                                       color = "black",
                                       weight = 2) # width border pixels

# Add each center to the map as a different layer
parks_layer <- addMarkers(map, data = parks, 
                                   long = ~longitude, lat = ~latitude,
                                   color = "green")
educational_centers_layer <- addMarkers(map, data = educational_centers,
                                                 lng = ~longitude, lat = ~latitude,
                                                 color = "blue")
health_centers_layer <- addMarkers(map, data = health_centers,
                                            lng = ~longitude, lat = ~latitude,
                                            color = "orange")



# Add layer controls to enable the user to control them
map <- map %>% addLayersControl(
  overlayGroups = c("parks_layer", "educational_centers_layer",
                    "health_centers_layer"),
